...
from rushstack.openstack.common import importutils
...
class AppFactory(BasePasteFactory):
    ...
    This requires rushstack.app_factory to be set to a callable which returns a
    WSGI app when invoked. The format of the name is <module>:<callable> e.g.

      [app:apiv1app]
      paste.app_factory = rushstack.common.wsgi:app_factory
      rushstack.app_factory = rushstack.api.cfn.v1:API

    The WSGI app constructor must accept a ConfigOpts object and a local config
    dict as its two arguments.
    """

    KEY = 'rushstack.app_factory'
...
class FilterFactory(AppFactory):
    ...
    This requires rushstack.filter_factory to be set to a callable which returns a
    WSGI filter when invoked. The format is <module>:<callable> e.g.

      [filter:cache]
      paste.filter_factory = rushstack.common.wsgi:filter_factory
      rushstack.filter_factory = rushstack.api.middleware.cache:CacheFilter

    The WSGI filter constructor must accept a WSGI app, a ConfigOpts object and
    a local config dict as its three arguments.
    """

    KEY = 'rushstack.filter_factory'
