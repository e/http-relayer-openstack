...
logger = logging.getLogger('rushstack.common.keystoneclient')
...
class KeystoneClient(object):
    ...
    def create_stack_user(self, username, password=''):
        ...
        the rushstack_stack_user_role as defined in the config
        ...
        # rushstack-generated "stack users" which will generally have credentials
        # deployed on an instance (hence are implicitly untrusted)
        roles = self.client.roles.list()
        stack_user_role = [r.id for r in roles
                           if r.name == cfg.CONF.rushstack_stack_user_role]
        if len(stack_user_role) == 1:
            ...
            logger.error("Failed to add user %s to role %s, check role exists!"
                         % (username,
                            cfg.CONF.rushstack_stack_user_role))
